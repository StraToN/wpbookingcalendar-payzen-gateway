...
class paymentFormToolbox
{
	 ...
	 *
	 * @param $fields array
	 *        	fields to send in the payment form
	 * @return string
	 */
	public function getSignature($fields)
	{
		...
		foreach ($fields as $nom => $valeur) {
			if (substr($nom, 0, 5) == 'vads_') {
				// Concatenation with "+"
				$signature_content .= stripcslashes($valeur) . "+";
			}
		}
		...
		$signature = base64_encode(hash_hmac('sha256', $signature_content, $this->certificate, true));
		return $signature;
	}
	 ...
	 *
	 * @param $fields array,
	 *        	the data received from Systempay, usually the _POST PHP super-global
	 *
	 * @return array|bool
	 */
